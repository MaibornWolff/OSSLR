...
describe("removeOverheadFromCopyright", function () {
  it("should remove html tags", function () {
    assert.equal(
      removeOverheadFromCopyright("Copyright <div> 2019 </div> Max Mustermann"),
      "Copyright 2019 Max Mustermann"
    );
    assert.equal(
      removeOverheadFromCopyright("<http://website.de> Hello <> World"),
      "Hello World"
    );
  ...
  it("should preserve the (c) symbol", function () {
    ...
    assert.equal(
      removeOverheadFromCopyright("(ccc) Copyright (C)"),
      "Copyright (C)"
    );
  });
  ...
  it("should remove all unnecessary whitespace", function () {
    assert.equal(
      removeOverheadFromCopyright("  first  (c)    second   "),
      "first (c) second"
    );
    assert.equal(
      removeOverheadFromCopyright("<  >  () copyright "),
      "copyright"
    );
  });
});
