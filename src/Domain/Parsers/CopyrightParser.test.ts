...
describe("extractCopyright", function () {
  ...
  it("should extract Copyright notices including (c) annotation", function () {
    assert.equal(
      copyrightParser.extractCopyright("Not this copyright (c) Owner Name"),
      "copyright (c) Owner Name"
    );
    assert.equal(
      copyrightParser.extractCopyright("More Text \n(C) Copyright 2020"),
      "(C) Copyright 2020"
    );
    assert.equal(
      copyrightParser.extractCopyright("© Copyright"),
      "© Copyright"
    );
  });
  it("should extract Copyright notices including a year specification", function () {
    assert.equal(
      copyrightParser.extractCopyright("copyright 2019-2020"),
      "copyright 2019-2020"
    );
    assert.equal(
      copyrightParser.extractCopyright("Text \nCopyright 2020 Owner"),
      "Copyright 2020 Owner"
    );
    assert.equal(
      copyrightParser.extractCopyright("Copyright © 2020"),
      "Copyright © 2020"
    );
  });
});
...
describe("removeOverheadFromCopyright", function () {
  ...
  it("should remove html tags", function () {
    assert.equal(
      copyrightParser.removeOverheadFromCopyright(
        "Copyright <div> 2019 </div> Max Mustermann"
      ),
      "Copyright 2019 Max Mustermann"
    );
    assert.equal(
      copyrightParser.removeOverheadFromCopyright(
        "<http://website.de> Hello <> World"
      ),
      "Hello World"
    );
    assert.equal(
      copyrightParser.removeOverheadFromCopyright("<This should be removed>"),
      ""
    );
  });
  ...
  it("should preserve the (c) symbol", function () {
    assert.equal(
      copyrightParser.removeOverheadFromCopyright("(c) Copyright"),
      "(c) Copyright"
    );
    assert.equal(
  ...
  it("should remove all URLs", function () {
    assert.equal(
      copyrightParser.removeOverheadFromCopyright(
        "copyright (C) http://www.google.com/?search=531"
      ),
      "copyright (C)"
    );
    assert.equal(
      copyrightParser.removeOverheadFromCopyright(
        "copyright www.github.com/user/repo#readme.md Owner"
      ),
      "copyright Owner"
    );
  });
  it("should remove everything after special characters", function () {
    assert.equal(
      copyrightParser.removeOverheadFromCopyright(
        "Copyright (c) 2022 * this should be removed"
      ),
      "Copyright (c) 2022"
    );
    assert.equal(
